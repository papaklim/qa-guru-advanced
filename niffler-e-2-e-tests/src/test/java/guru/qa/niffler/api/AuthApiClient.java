...
public class AuthApiClient {
  ...
  private final Retrofit retrofit = new Retrofit.Builder().baseUrl(
    "https://auth.niffler-stage.qa.guru/").addConverterFactory(JacksonConverterFactory.create()).client(
  ...
  public Response<Void> register(String username, String password) throws IOException {
    ...
    return authApi.register(
      ...
      password,
      password,
      cm.getCookieStore().getCookies().stream().filter(
        c -> c.getName().equals("XSRF-TOKEN")).findFirst().get().getValue()
    ).execute();
  }
}
